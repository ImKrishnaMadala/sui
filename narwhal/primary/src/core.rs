...
use network::{
    anemo_ext::NetworkExt, CancelOnDropHandler, P2pNetwork, PrimaryToPrimaryRpc, ReliableNetwork,
    UnreliableNetwork,
};
...
use types::{
    ...
    Certificate, CertificateDigest, Header, HeaderDigest, LatestHeaderRequest,
    PrimaryToPrimaryClient, ReconfigureNotification, RequestVoteRequest, Round,
...
pub struct Core {
    ...
    /// Receiver for certificates. TODO
...
impl Core {
    ...
    async fn propose_header(
        ...
        header_store: Store<HeaderDigest, Header>,
        certificate_store: CertificateStore,
        mut signature_service: SignatureService<Signature, { crypto::DIGEST_LENGTH }>,
        network: anemo::Network,
        header: Header,
        ...
        // Process the header.
        // TODO-DNS - any need to verify self-produced header? Guessing not?
        // TODO-DNS increment headers recvd metric, headers processed metric for own header?
        header_store
        ...
        // TODO-DNS do we need any protection here against equivocation on self-produced headers?
        ...
        loop {
            ...
            tokio::select! {
                ...
                _ = &mut cancel => return Err(DagError::Canceled),
            }
        }
    ...
    async fn process_own_certificate(&mut self, certificate: Certificate) -> DagResult<()> {
        ...
        let network_keys = self
            ...
            .map(|(_, _, network_key)| network_key)
            .collect();
        // TODO-DNS is unreliable broadcast ok here? I think so but double check.
    ...
    async fn process_certificate_internal(&mut self, certificate: Certificate) -> DagResult<()> {
        ...
        self.batch_sync_tasks
            .spawn(async move { synchronizer.sync_batches(&header, network).await });
    ...
    async fn change_epoch(&mut self, committee: Committee) {
        ...
        // TODO-DNS move to cleanup in PrimaryReceiverHandler
